...
Java JDK 8 (or later) and Maven are required for building the project. 
...
To use the Core component in the other projects, you'll need to install it locally by:
...
To test the application on top of a MYSQL installation, you need to change [`database.properties`][database.prop] and run the Docker container in the root folder with:
...
It is necessary a user interface to run Open Hospital. Interfaces are provided in the [GUI][openhospital-gui] and in the [UI][openhospital-ui] projects.
Please follow the instructions onthe documentation pages of those repositories.
